...
namespace BackupAzureDevops
{
    ...
    public static class BackupAzureDevops
    {
        private const string version = "api-version=5.1";

        [FunctionName("BackupAzureDevops")]
        public static async void Run([TimerTrigger("* 5 * * * *"
#if DEBUG 
,RunOnStartup=true // When debugging... run the job as soon as we press debug, no need to wait for the timer. 
            ...
            )]TimerInfo myTimer, ILogger log) 
        {
            ...
            // log.LogInformation($"accountName: {blobServiceClient.AccountName}");
            ...
            foreach (Project project in projects.value)
            {
                ...
                foreach (Repo repo in repos.value)
                {
                    ...
                    if (items.count > 0)
                    {
                        ...
                        if (counter <= 0)
                        {
                            log.LogInformation($"upload: {name} done");
                            return;
                        }
